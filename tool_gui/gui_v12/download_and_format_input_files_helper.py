...
def download_weather_data_runner(settings, gridded_data_array_size, widget):
    fmi_attempts = 0
    while fmi_attempts <= FMI_STATION_LIMIT:
        ...
        widget.insert(tkinter.END, f"Downloading weather data from FMI...(attempt #{fmi_attempts+1})\n")
        start_downloading_time = time.time()
        try:
            weather_daily_data = download_weather_data(settings.coordinates, settings.start_time, settings.end_time, time_format="%Y-%m-%d %H:%M", nearest_station=fmi_attempts)
        except IndexError:
            # logger.exception("No data available for this weather station!")
            widget.insert(tkinter.END, "No data available for this weather station!\n")
            fmi_attempts += 1
            continue

        end_downloading_time = time.time()
        widget.insert(tkinter.END, "Weather data downloaded in {} s\n".format(end_downloading_time-start_downloading_time))

        if weather_daily_data.index.size != gridded_data_array_size:
            widget.insert(tkinter.END, "Gridded FMI and weather data have different number of total days!\n")
            fmi_attempts += 1
            continue
    ...
    if no_data_first > NO_DATA_WEATHER_THRESHOLD:
        widget.insert(tkinter.END, "Downloading weather data from FMI again using the next closest station...\n")
        start_downloading_time = time.time()
        weather_daily_data_second = download_weather_data(settings.coordinates, settings.start_time, settings.end_time, time_format="%Y-%m-%d %H:%M", nearest_station=fmi_attempts+1)
        end_downloading_time = time.time()
        widget.insert(tkinter.END, "Weather data downloaded in {} s\n".format(end_downloading_time-start_downloading_time))

        no_data_second = calc_percent_unavailable_data(weather_daily_data_second)
        if no_data_second > no_data_first:
            widget.insert(tkinter.END, "Next closest station have even less data available. Switching back to the previous station.\n")
        else:
